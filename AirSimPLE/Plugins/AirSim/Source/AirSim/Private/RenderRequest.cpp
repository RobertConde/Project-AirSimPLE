    ...
    for (unsigned int i = 0; i < req_size; ++i)
    {
        if (!params[i]->pixels_as_float)
        {
            if (results[i]->width != 0 && results[i]->height != 0)
            {
                ...
                {
                    TArrayView64<const FColor> SrcView(results[i]->bmp.GetData(), results[i]->bmp.Num());
                    TArray64<uint8> DstData64;
                    FImageUtils::PNGCompressImageArray(results[i]->width, results[i]->height, SrcView, DstData64);
                    results[i]->image_data_uint8.SetNumUninitialized(DstData64.Num());
                    FMemory::Memcpy(results[i]->image_data_uint8.GetData(), DstData64.GetData(), DstData64.Num());
                }
            }
        }
        else
